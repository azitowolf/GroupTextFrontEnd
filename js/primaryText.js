...
//AJAX to delete ptext
  $('#ptexts').on('click', '#deletePtext', function(){
    ...
    var t = $(this);
    $.ajax({
      url: 'http://localhost:3000/ptexts/' + $id,
      headers: { Authorization: 'Token token=' + currentToken},
      type: 'DELETE',
      dataType: 'json',
    })
  ...
  $('#new-post-button').click(function(event) {
    event.preventDefault();
      $.ajax({
        ...
        headers: { Authorization: 'Token token=' + currentToken},
        dataType: 'json',
        data: {ptext: {history: $('#textHistory').val(), text: $('#currentText').val()}
      }
      })
      .done(function(contents) {
        console.log(contents);
        var newP = makePtext(contents.ptext.id, contents.ptext.image, contents.ptext.user.name, contents.ptext.text);
        $('#ptexts').append(newP);
